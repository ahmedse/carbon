# Generated by hand for Phase P1C — Dataset Hub adoption from the datahub app.
#
# Cross-app model move (datahub → catalog), data-preserving:
#   * database_operations: RENAME the 6 datahub_* tables (+ M2M through table
#     datahub_dataset_tags) to catalog_* names. RunSQL is used because Django's
#     AlterModelTable is 2-arg and resolves the OLD table name from the model in
#     the migration's own app state — the models live in the datahub app's
#     state here, so AlterModelTable cannot be used from the catalog app.
#     The renames are explicit old → new, matching the spec's intended
#     "AlterModelTable(name, old_table, new_table)" semantics 1:1.
#   * state_operations: CreateModel for all 6 models in catalog's state,
#     transcribed field-for-field from datahub 0001_initial + 0002 (the
#     authoritative auto-generated state), with FK targets repointed from
#     'datahub.X' to 'catalog.X'. The M2M through table (Dataset_tags) is
#     created implicitly in state by CreateModel(Dataset).
#
# After this migration: tables are catalog_*, catalog's state owns the models,
# and no fresh tables are created in the DB.
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.SeparateDatabaseAndState(
            database_operations=[
                # ── Table renames: datahub_* → catalog_* (no data movement) ──
                migrations.RunSQL(
                    'ALTER TABLE "datahub_dataset" RENAME TO "catalog_dataset"',
                    reverse_sql='ALTER TABLE "catalog_dataset" RENAME TO "datahub_dataset"',
                ),
                migrations.RunSQL(
                    'ALTER TABLE "datahub_datasetversion" RENAME TO "catalog_datasetversion"',
                    reverse_sql='ALTER TABLE "catalog_datasetversion" RENAME TO "datahub_datasetversion"',
                ),
                migrations.RunSQL(
                    'ALTER TABLE "datahub_datasetversionmember" RENAME TO "catalog_datasetversionmember"',
                    reverse_sql='ALTER TABLE "catalog_datasetversionmember" RENAME TO "datahub_datasetversionmember"',
                ),
                migrations.RunSQL(
                    'ALTER TABLE "datahub_datacontract" RENAME TO "catalog_datacontract"',
                    reverse_sql='ALTER TABLE "catalog_datacontract" RENAME TO "datahub_datacontract"',
                ),
                migrations.RunSQL(
                    'ALTER TABLE "datahub_datacontractviolation" RENAME TO "catalog_datacontractviolation"',
                    reverse_sql='ALTER TABLE "catalog_datacontractviolation" RENAME TO "datahub_datacontractviolation"',
                ),
                migrations.RunSQL(
                    'ALTER TABLE "datahub_datasetaccesspolicy" RENAME TO "catalog_datasetaccesspolicy"',
                    reverse_sql='ALTER TABLE "catalog_datasetaccesspolicy" RENAME TO "datahub_datasetaccesspolicy"',
                ),
                # M2M through table for Dataset.tags (auto-created in datahub 0001)
                migrations.RunSQL(
                    'ALTER TABLE "datahub_dataset_tags" RENAME TO "catalog_dataset_tags"',
                    reverse_sql='ALTER TABLE "catalog_dataset_tags" RENAME TO "datahub_dataset_tags"',
                ),
            ],
            state_operations=[
