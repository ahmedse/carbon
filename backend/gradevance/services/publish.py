...
def engine_sg_labels_for_held_out(device_pack_rel: str) -> tuple[list[str], list[str]]:
    ...
    for row in rows:
        for seg in row.get("segments") or []:
            expert.append(seg.get("sg_label") or f"N{seg.get('sg_numeric')}")
            draft = _SegDraft(
            ...
            sg = next((c for c in codes if c["dimension"] == "semantic_gravity"), None)
            engine.append((sg or {}).get("value") or "SG-")
...
def evaluate_publish_gate(
    ...
    *assignment* is summative even if the pack file defaults to formative.
    ...
    rel = passes_reliability_gate(expert, engine, minimum=min_k) if expert else canary.reliability
    return {
        "required": True,
        "passed": canary.allowed,
