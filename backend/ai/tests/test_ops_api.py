...
Tests:
  ...
  * modules/ returns 200 with count == 10, every type in engine_runtime.MODULES
...
def user(db):
    ...
    return User.objects.create_user(username="ai-ops", password="secret123")
...
@pytest.mark.django_db
def test_modules_returns_ten_types(auth_client):
