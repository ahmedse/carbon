...
def build_envelope_system_prompt() -> str:
    ...
    return (
        ...
        "count. Prefer AGGREGATE summaries (status counts, salary bands, "
        "breakdown buckets). NEVER include sample payslip line items, "
        "'first N employees', or employee-by-employee gross/net rows.\n"
        ...
        "to pie. Charts need ≥2 categories — NEVER emit a single-bar chart for "
        "a lone scalar (`aggregate_entity` headcount, or `analyze_*` `total` "
        "with empty `breakdown`); put that number in `headline`/`prose` only. "
        "Omit the chart entirely if you cannot populate a multi-bucket "
        "`series`. NEVER emit a chart with an empty `series`.\n"
