...
def _coerce_export_steps(steps: list[PlanStep]) -> None:
    ...
    'excel'/'workbook'/'spreadsheet' → xlsx, otherwise both. Content/table/title
    are generated by the LLM at execution from the prior findings (depends_on).
    """
    for step in steps:
        if step.tool_name:
            continue
        ...
        if not _EXPORT_DOC_INTENT.search(intent):
            continue
        wants_word = bool(re.search(r"\b(?:word|docx|\.doc)\b", intent))
        wants_excel = bool(re.search(r"\b(?:excel|xlsx|spreadsheet|workbook|\.xls)\b", intent))
