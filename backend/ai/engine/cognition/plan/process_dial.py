...
* **Process dial owns the DAG spine** — which steps exist, consent, capabilities.
  Personal leave never LLM-invents a random balance+submit topology.
* **Deterministic slot fill owns codes/dates** — ``fill_write_body`` from the brief.
* **LLM is residual intelligence** — not used for the leave spine today; reserved
  for future title/clarify when slots stay empty after grounding. Consent UI
  still asks only truly missing governed fields.
* **Host Plane A owns review** — ``leave.request.lifecycle`` review/record/verify
  stay Correspondence / host SoD. Pulse Run covers the AI-runnable prefix:
  optional entitlement check → submit (act_confirm).

See ADR-0045 (planes), ADR-0046 (Chat handoff), QA PA-030 / E-LV-*.
"""
...
def is_personal_leave_brief(utterance: str) -> bool:
    ...
    if not text:
        return False
    # Compliance / board-pack stays on freeform LLM planning.
    if _LEAVE_COMPLIANCE.search(text):
        return False
    if _LEAVE_PERSONAL.search(text) or _BARE_LEAVE.search(text):
        return True
    # Broader first-person leave (e.g. "I want one day of annual leave…")
    # that scope_route's tighter verb+leave adjacency can miss.
    return bool(
        re.search(
...
def materialize_leave_request_plan(
    ...
    *,
    ...
    Returns a :class:`~ai.engine.cognition.plan.planner.Plan` with
    ``source="process_dial"``.
    """
    ...
    # Phase A — observe entitlement (process ask_if: days vs remaining).
    ...
    # Phase B — governed submit (YAML step ``submit``, autonomy act_confirm).
    ...
    submit_step = PlanStep(
        step_id=1,
        intent=_submit_intent(brief, body),
        tool_name="call_host_api",
    ...
    grounded = [k for k in ("leave_type", "start_date", "end_date", "days") if body.get(k) not in (None, "")]
    missing = [k for k in ("leave_type", "start_date", "end_date", "days") if k not in grounded]
...
def _submit_intent(brief: str, body: dict[str, Any]) -> str:
    ...
    # Keep a short echo of the brief when we have almost nothing grounded.
