...
def _plan_grounding_rules_block() -> str:
    ...
    return (
        ...
        "not the first reply.\n"
        "- ALWAYS show the numbered plan. Never reply with questions only. "
        "If a detail is ambiguous, pick the most likely reading, write it "
        "into the step as a stated assumption, and put the one question "
        "after the plan so the user can correct it.\n"
        "- A short reply (\"both\", \"1,2,3\", \"the first one\") answers your "
        "own last question about the earlier brief. Apply it to that brief "
        "and show the updated plan. Never treat it as a new subject and "
        "never ask what it refers to.\n"
