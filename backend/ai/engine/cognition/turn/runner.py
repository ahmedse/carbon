    ...
    async def run(
        ...
        if draft_tools:
            system_prompt = (
                ...
                "- For deterministic rule types (not_null, unique, "
                "allowed_values, range, regex), the rule must bind to a "
                "specific field. If the user hasn't named the field/column, "
                "ask which field to check BEFORE calling create_dq_rule — "
                "never call it without a field. Do not offer a duplicate "
                "check instead of asking for the field.\n"
