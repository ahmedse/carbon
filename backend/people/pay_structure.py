"""Committed-pay structure: one named aggregate, closed query, declared fields.

Population: PayslipLine rows on committed runs for one period_end and one
line_type (net|gross), one amount per employee, grouped by one employee
dimension. The host computes every number. Pulse restates.
"""
from __future__ import annotations

from collections import defaultdict
from decimal import Decimal, ROUND_HALF_UP
from typing import Any

from django.utils.dateparse import parse_date

from .models import PayrollRun, PayslipLine
from .permissions import is_global_admin

DIMENSIONS = ("org_unit", "nationality", "position", "is_active")
LINE_TYPES = ("net", "gross")
QUANT = Decimal("0.001")
RETURNS = (
    "label", "headcount", "average", "median", "min", "max", "total",
    "period_end", "dimension", "line_type", "status",
)


def _money(value: Decimal) -> str:
    return str(value.quantize(QUANT, rounding=ROUND_HALF_UP))


def _median(amounts: list[Decimal]) -> Decimal:
    ordered = sorted(amounts)
    n = len(ordered)
    mid = n // 2
    if n % 2:
        return ordered[mid]
    return (ordered[mid - 1] + ordered[mid]) / 2


def _label(employee, dimension: str) -> str:
    if dimension == "org_unit":
        unit = getattr(employee, "org_unit", None)
        return (getattr(unit, "name", None) or "").strip() or "(blank)"
    if dimension == "nationality":
        ref = getattr(employee, "nationality", None)
        return (getattr(ref, "code", None) or "").strip() or "(blank)"
    if dimension == "position":
        pos = getattr(employee, "position", None)
        return (getattr(pos, "title", None) or "").strip() or "(blank)"
    return "active" if getattr(employee, "is_active", False) else "inactive"


def _org_ids(user) -> list[int] | None:
    if is_global_admin(user):
        return None
    from .views import _visible_org_unit_ids

    return list(_visible_org_unit_ids(user))


def summarize_committed_pay(
    user,
    *,
    period_end: str,
    dimension: str,
    line_type: str = "net",
) -> tuple[int, dict[str, Any]]:
    """``(status, payload)`` for the closed pay-structure GET."""
    dim = (dimension or "").strip()
    if dim not in DIMENSIONS:
        return 400, {
            "detail": f"Unknown dimension {dimension!r}. Allowed: {', '.join(DIMENSIONS)}",
        }
    kind = (line_type or "net").strip() or "net"
    if kind not in LINE_TYPES:
        return 400, {
            "detail": f"Unknown line_type {line_type!r}. Allowed: {', '.join(LINE_TYPES)}",
        }
    parsed = parse_date((period_end or "").strip())
    if parsed is None:
        return 400, {"detail": "period_end is required (YYYY-MM-DD)."}

    org_ids = _org_ids(user)
    runs = PayrollRun.objects.filter(status="committed", period_end=parsed)
    if org_ids is not None:
        runs = runs.filter(org_unit_id__in=org_ids)
    run_ids = list(runs.values_list("id", flat=True))

    empty = {
        "period_end": parsed.isoformat(),
        "dimension": dim,
        "line_type": kind,
        "status": "committed",
        "run_ids": run_ids,
        "omitted": 0,
        "caveats": ["No committed pay lines for this period and dimension."],
        "breakdown": [],
    }
    if not run_ids:
        return 200, empty

    lines = (
        PayslipLine.objects.filter(
            payroll_run_id__in=run_ids,
            line_type__code=kind,
        )
        .select_related(
            "employee",
            "employee__org_unit",
            "employee__nationality",
            "employee__position",
        )
    )
    if org_ids is not None:
        lines = lines.filter(employee__org_unit_id__in=org_ids)

    by_employee: dict[int, list] = defaultdict(list)
    for line in lines:
        by_employee[line.employee_id].append(line)

    omitted = 0
    grouped: dict[str, list[Decimal]] = defaultdict(list)
    for emp_lines in by_employee.values():
        if len(emp_lines) != 1:
            omitted += 1
            continue
        line = emp_lines[0]
        grouped[_label(line.employee, dim)].append(line.amount)

    breakdown = []
    for label in sorted(grouped):
        amounts = grouped[label]
        total = sum(amounts, Decimal("0"))
        count = len(amounts)
        breakdown.append({
            "label": label,
            "headcount": count,
            "average": _money(total / count),
            "median": _money(_median(amounts)),
            "min": _money(min(amounts)),
            "max": _money(max(amounts)),
            "total": _money(total),
        })

    caveats = []
    if omitted:
        caveats.append(
            f"{omitted} employee(s) omitted: not exactly one {kind} line in the period."
        )
    if not breakdown:
        caveats.append("No committed pay lines for this period and dimension.")

    return 200, {
        "period_end": parsed.isoformat(),
        "dimension": dim,
        "line_type": kind,
        "status": "committed",
        "run_ids": run_ids,
        "omitted": omitted,
        "caveats": caveats,
        "breakdown": breakdown,
    }
