...
RULE_33=GRADEVANCE PROFESSOR JOURNEY IA (hard rule, 2026-09-18) — Professor UX follows Course → Assignment(stem) → Submission → Run → Calibration → Proposals. Primary surfaces: Courses & stems, Assignment hub, Run workbench (HITL), Calibration, Marking queue, Proposals. Student desk is student-facing only — never the cohort admin home. Overview deep-links must not dump professors into Student desk for runs. Canonical: docs/eduos/GRADEVANCE-PROFESSOR-JOURNEY.md · GRADEVANCE-PROFESSOR-LIFECYCLE.md.
