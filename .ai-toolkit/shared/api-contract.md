...
## AI-Specific Envelope Rules

**Reference**: `.ai-toolkit/shared/ai-contract.md` — THE binding AI contract.
AI operations follow all standard envelope rules above, plus these AI-specific constraints:

### AI Endpoint Routing

All AI calls go through `CarbonIntelligence` (Wave C), never raw views.
- `CarbonIntelligence` → AIProvider (swappable via `AI_PROVIDER_CLASS`)
- AI tasks dispatched via `POST /tasks` with `{ "module": "...", "payload": {...} }`
- Every AI call carries a `Scope` (user context) — see `backend/ai/protocol.py`

### Async AI Operations

- Long-running AI work (DQ batch validation, anomaly detection, report generation):
  - `POST` starts the job → return `202` + `{ "run_id": "..." }`
  - `GET /tasks/{run_id}/` → `{ "status": "running|done|failed", "result": {...} }`
  - Frontend polls the status endpoint. NEVER block the request thread.

### AI Error Response Shape

AI errors MUST follow the same shape as all other endpoints:
```json
{ "detail": "Human-readable message" }
// ALWAYS use this. NEVER return raw tracebacks or provider-internal errors.
```

If the AI provider is unavailable, return:
```json
{ "detail": "AI provider unavailable", "code": "ai_provider_unavailable" }
```
Status code MUST be `503` (Service Unavailable), not `500`.

### AI Response Data Wrapping

AI responses from the provider are ALWAYS wrapped by `CarbonIntelligence`.
The frontend NEVER receives raw provider output. The wrapper:
- Sanitizes error messages (no provider internals leaked)
- Adds audit trail metadata (see §7 of ai-contract.md)
- Ensures data isolation (no cross-app data leakage)

### AI-Specific HTTP Status Codes

| Code | When |
|------|------|
| 202 | AI task accepted, processing asynchronously |
| 422 | AI request semantically invalid (bad scope, missing required fields) |
| 503 | AI provider unavailable (graceful degradation — see ai-contract.md §6) |

### Provider Swappability

The frontend MUST NOT know which AI provider is active.
No provider-specific fields in API responses. No provider-specific error codes.
Swapping providers is a backend config change only (`AI_PROVIDER_CLASS`).
