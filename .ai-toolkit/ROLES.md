...
**Cursor-native dispatch (2026-09-22):** when the Master Architect session runs inside Cursor, workers are dispatched as **Task subagents** instead of Zoo Code pastes. Same tiering principle — Master = the session model, workers = the **cheapest capable model on the Cursor roster**: `composer-2.5-fast` for mechanical backend/frontend/QA edits, `claude-4.5-haiku-thinking` for design-heavy test/eval authoring or debugging. **Escalation ladder (2026-09-22, PV2-0A/0B lesson):** if a cheap worker's output is rejected once for RULE_28 (thin/broken measurement, dead hooks, xfail masking infra errors), the rev2 dispatch escalates to `claude-opus-5-5-medium`; edits to `engine/cognition/turn/runner.py` or `engine/cognition/plan/loop.py` start at opus. Never re-dispatch the same cheap model twice for the same defect class. The worker protocol is unchanged: the subagent prompt must tell it to read `project.config.md` + `shared/base-rules.md` + its role file + the exact TASKS.md phase, run the phase's verification gate, and append to `TASK-RESULTS.md`. The Master audits by reading the diff and **re-running the gate itself** before flipping a phase to DONE (`roles/master-architect.md` → Reviewing Worker Output). Parallel workers only on disjoint file sets (no worktrees — shared `.venv` + test DB).
