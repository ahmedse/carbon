...
- Follow-up (2026-09-12): PB-24's ErrorBoundary fix stops the CRASH but leaves a gap — `React.lazy` caches the REJECTED import forever, so after the transient failure the palette stays broken (and the ErrorBoundary "Try again" button can't recover it, because the factory is never re-run) until a full page reload. Fix: `src/utils/lazyWithRetry.js` (`withRetry` + `lazyWithRetry` — exponential backoff, injectable `_sleep` for tests) used for the always-mounted `CommandPalette` import in `Shell.jsx`. Regression test: `src/__tests__/lazyWithRetry.test.js` (4 cases: first-try success, transient-failure retry, exhaust-then-reject, backoff timing). Follow-up backlog: the ~100 route-level `React.lazy()` imports in `App.jsx` (and `AIMessageBubble`/`MyDashboard` component-level ones) could also adopt `lazyWithRetry` — not done here to keep the fix minimal.
