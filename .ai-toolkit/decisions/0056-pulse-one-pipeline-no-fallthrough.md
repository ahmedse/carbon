...
3. **Rewrites become failures.** `_execute_bound_read` fails the read visibly on ungrounded numbers (`Degradation("speak", "ungrounded")`) instead of deleting them. FallbackHandler is not reachable from Chat.
...
|---|---|---|---|
| 1 | `answer` / `navigate` / `set_slot` / plan-handoff finished inside v21; fallthrough becomes a typed error behind a counter | No | G6, G5, new "no-fallthrough" bank |
