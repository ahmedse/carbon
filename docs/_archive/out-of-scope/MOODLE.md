...
`config.php` is **generated by `admin/cli/install.php`** (it writes the file itself and refuses to run if one already exists), then **hardened by `deploy.sh`**, which injects the air-gap/security block below before the `require_once(__DIR__ . '/lib/setup.php')` line (see `deploy/moodle/harden/config-extra.php`). The non-negotiable settings that end up in the file:
...
// Air-gap (injected by deploy.sh from harden/config-extra.php)
...
> `passwordsaltmain` is added by `deploy.sh` if the installer omitted one; when added *after* install, the admin password is re-hashed with `reset_password.php` to match.
