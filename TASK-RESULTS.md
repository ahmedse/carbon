...
## PV21-ROW9-PLAN-REVISION — 2026-09-24

- Incident: Chat/Plan dial, "refine plan…" → "apply" → "accept" ended in "I wasn't able to generate a response". Root cause was four layers, not one bug: an apply-phrase allowlist (`_AGENT_DISCUSS_APPLY_PHRASES`/`_SHORT`) invisible to the re.compile meter, transcript-marker scanning for Discuss continuity, a plan-id `re.search` over history, and a DISCUSS-APPLY prompt ordering `edit_plan` that the ADR-0046 guard correctly cancels on every Chat surface. All four deleted.
- Replacement (no new gate, no new `StagedExit`): the Discuss reply becomes a typed `plan_revision` open_question (`turn/plan_revision.py`); the shared affirmation module resolves it (`is_commit_affirmation`: apply / accept / commit / yes / نعم / اعتمدها / طبقها — not "apply for three days leave", not "do not apply yet"); the confirm turn returns a 0-LLM `open_panel` handoff carrying `revision` to the Tasks panel, where a "Revision proposed in Chat" card calls the existing `handleReplanPlan` diff review (RULE_21). Chat proposes, Agent applies. Golden `test_plan_revision_state.py` incl. `test_edit_plan_in_chat_plan_is_cancelled_by_adr_0046_guard`.
- Gauge (the user's ask: is intelligence advancing and measured?): `ai.eval.pulse_gauge` writes an instance-scoped snapshot + series (`docs/pulse/evidence/PV2-gauge-series.json`, `PV2-gauge-2026-09-24.json`) and `--gate` fails on any engine meter rising vs the previous row, any ladder regression, any pack violation. Ceiling file is now `max(target, measured)` per key (it was toothless). `intelligence_ladder` L6 reads the newest G6 file and L7 reads `harness_budget.measure()` live instead of a dated file.
